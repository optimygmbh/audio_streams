...
public class SwiftAudioStreamsPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        if(call.method == "initialize"){
            ...
            case "AVAudioCommonFormat.pcmFormatInt16":
                self.outputFormat = AVAudioFormat(commonFormat: AVAudioCommonFormat.pcmFormatInt16,
            ...
            case "AVAudioCommonFormat.pcmFormatInt32":
                self.outputFormat = AVAudioFormat(commonFormat: AVAudioCommonFormat.pcmFormatInt32,
                                             sampleRate: args["sampleRate"] as! Double,
                                             channels: args["channelCount"] as! UInt32,
                                             interleaved: args["interleaved"] as! Bool
                )
    ...
    public func onListen(withArguments arguments: Any?, eventSink events: @escaping FlutterEventSink) -> FlutterError? {
        ...
        input.installTap(onBus: bus, bufferSize: 1024, format: inputFormat) { (buffer, time) -> Void in
            ...
            let outputSampleRate = self.outputFormat!.sampleRate
            let bufferSampleRate = buffer.format.sampleRate
            let bufferFrameLength = buffer.frameLength
            
            let outputFrameCount = AVAudioFrameCount(outputSampleRate)
            let bufferFrameCount = AVAudioFrameCount(bufferSampleRate)
            
            let frameCapacity = outputFrameCount * bufferFrameLength / bufferFrameCount
            
            let convertedBuffer = AVAudioPCMBuffer(pcmFormat: self.outputFormat!, frameCapacity: frameCapacity)!
            ...
            if (self.outputFormat?.commonFormat == AVAudioCommonFormat.pcmFormatInt16) {
                let data = convertedBuffer.int16ChannelData![0];
